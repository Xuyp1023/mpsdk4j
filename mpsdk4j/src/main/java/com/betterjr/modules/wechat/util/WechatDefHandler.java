...
public class WechatDefHandler implements WechatHandler {
    ...
    private WechatMediaService  mediaService = new WechatMediaService();
    ...
    public BasicMsg image(final ImageMsg msg) {
        final TextMsg text_msg = new TextMsg(msg);
        final boolean result = mediaService.saveWechatMedia(msg);
        if (result) {
            text_msg.setContent("您上传的信息已经保存！");
        }
        else {
            text_msg.setContent("上传信息保存失败！");
        }
